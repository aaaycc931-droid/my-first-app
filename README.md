...
- 本机谱项目已有首个正式受控导入入口：支持明确的单声部 MusicXML／MXL 子集，先生成内存候选和 blocking ledger，用户确认后才原子新增保存；`part-list`、`score-part`、`part-name` 与内容 `part` 的身份载体采用 exact namespace／大小写／属性／纯文本门禁，ID 与名称不做静默 trim。
