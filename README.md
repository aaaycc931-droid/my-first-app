...
- 本机谱项目已具备同一严格子集的 MusicXML／MXL 受控导出：先生成内存候选和 blocking ledger，用户明确确认后才下载；当前严格子集已覆盖未指定或 canonical `0–127` GM1 program 乐器归属、note/rest 单附点、延长记号、`pp`–`ff` 受控力度记号、`down/up` 单事件制音踏板记号和自然／单升降根音七类受控和弦标记（major、minor、dominant、major-seventh、minor-seventh、augmented、diminished），pitched note 的规范化单段歌词、`1–5` 单指法和 accent／staccato／tenuto 单音演奏法、同声部相邻且时间连续 note 的圆滑线，以及同音高 note 的延音线 start／stop、跨小节和链式关系的仓库内部双向 round-trip。GM1 program 只映射固定 `score-instrument` 和一基 `midi-program 1–128`；channel、bank、program change、真实音色选择和 MIDI 文件仍未支持。re-import 与 legacy parser 交叉检查不等于第三方独立阅读器验收。
