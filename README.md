...
- 本机谱项目已具备同一严格子集的 MusicXML／MXL 受控导出：先生成内存候选和 blocking ledger，用户明确确认后才下载；当前严格子集已覆盖 note/rest 延长记号的仓库内部双向 round-trip，但 re-import 与 legacy parser 交叉检查不等于第三方独立阅读器验收。
...
2. 继续以 fail-closed 小切片扩大经无损验证的 MusicXML 子集；当前 note/rest 延长记号支持不代表完整 MusicXML。
