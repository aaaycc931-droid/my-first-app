...
新任务、跨账号继续和文档选择先从 [`docs/START-HERE.md`](docs/START-HERE.md) 开始；CI、PR、额度、APK artifact 与持久化交接规则见 [`docs/project-process-and-account-continuity.md`](docs/project-process-and-account-continuity.md)。
...
- 本机谱项目已有首个正式受控导入入口：支持明确的单声部 MusicXML／MXL 子集，先生成内存候选和 blocking ledger，用户确认后才原子新增保存；`part-list`、`score-part`、`part-name` 与内容 `part` 的身份载体采用 exact namespace／大小写／属性／纯文本门禁，ID 与名称不做静默 trim；每个 note/rest 的外层 `<note>` 也必须大小写精确、无 namespace 且无属性。
- 本机谱项目已具备同一严格子集的 MusicXML／MXL 受控导出：先生成内存候选和 blocking ledger，用户明确确认后才下载；当前严格子集已覆盖未指定或 canonical `0–127` GM1 program 乐器归属、note/rest 单附点、延长记号、`pp`–`ff` 受控力度记号、`down/up` 单事件制音踏板记号和自然／单升降根音二十四类受控和弦标记（major、minor、dominant、major-seventh、minor-seventh、major-sixth、minor-sixth、suspended-second、suspended-fourth、power、dominant-ninth、major-ninth、minor-ninth、dominant-11th、major-11th、minor-11th、dominant-13th、major-13th、minor-13th、augmented、diminished、augmented-seventh、diminished-seventh、half-diminished；共 504 个根音／类别组合），pitched note 的规范化单段歌词、`1–5` 单指法和 accent／staccato／tenuto 单音演奏法、同声部相邻且时间连续 note 的圆滑线，以及同音高 note 的延音线 start／stop、跨小节和链式关系的仓库内部双向 round-trip。下一交换语义仍为 `NEXT EXCHANGE SLICE UNDER REVIEW`。GM1 program 只映射固定 `score-instrument` 和一基 `midi-program 1–128`；channel、bank、program change、真实音色选择和 MIDI 文件仍未支持。re-import 与 legacy parser 交叉检查不等于第三方独立阅读器验收。
...
2. 继续以 fail-closed 小切片扩大经无损验证的 MusicXML 子集；当前单附点、单段歌词、单指法、单音演奏法、单事件力度记号、单事件制音踏板记号、受控和弦标记、延长记号、严格圆滑线和严格延音线支持不代表完整 MusicXML。
