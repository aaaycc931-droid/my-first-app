# S3 MusicXML／MXL 半减七和弦严格 round-trip 验收
...
- 在既有受控 `chordSymbol` 双向映射上只增加一种和弦类别：
  `Cm7b5` ↔ `<kind>half-diminished</kind>`。
...
- `Cø7`、`Cø` 或其他 alias 不得归一化为 `Cm7b5`。
- `Cdim7`／`diminished-seventh` 不得与 `Cm7b5`／`half-diminished` 混同。
- augmented-seventh、其他七和弦、sus／add／扩展音继续 blocking。
...
- 纯映射覆盖 half-diminished、全部 7 个自然根音及其单升／单降形式。
- XML 与 MXL 都以 pitched note 上的 `C#m7b5`、rest 上的 `Dbm7b5` 验证 exact
  import；导出 fixture 验证 `<kind>half-diminished</kind>` 和 exact re-import。
- `Cø7` alias、diminished-seventh、未知 kind、结构变体和所有既有 blocker 保持
  失败关闭。
