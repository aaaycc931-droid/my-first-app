...
- 当前严格子集已把 note/rest 的 canonical 单附点双向映射为直接、唯一且结构为空的 `<dot/>`，把 `fermataMark` 双向映射为无属性且无文本的 `<fermata/>`，把 note 的 `slurToNext` 双向映射为相邻时间连续 note 上严格配对的 `<slur type="start"/>`／`<slur type="stop"/>`，并把 `tieToNext` 双向映射为相邻、同音高且时间连续 note 上成对一致的 `<tie>`／`<tied>` start／stop；附点真实时值参与 duration、容量、拍位和关系连续性核对，同小节、跨小节、链式关系及与 fermata／slur 共存均保留。其他未列出的语义继续 blocking。
...
- 导入边界见 `docs/s3-local-score-project-musicxml-import-acceptance.md`，导出边界见 `docs/s3-local-score-project-musicxml-export-acceptance.md`；单附点、fermata、圆滑线和延音线双向边界分别见 `docs/s3-local-score-project-musicxml-augmentation-dot-round-trip-acceptance.md`、`docs/s3-local-score-project-musicxml-fermata-round-trip-acceptance.md`、`docs/s3-local-score-project-musicxml-slur-round-trip-acceptance.md` 与 `docs/s3-local-score-project-musicxml-tie-round-trip-acceptance.md`。浏览器真实下载、Android WebView／真机、第三方独立阅读器、真实音频、教师审核、MIDI、OMR、完整 MusicXML、S3 与正式版 V1 均仍为 `NOT_EXECUTED` 或未完成。
