...
- 最新产品功能基线：P116d / PR #394，合并提交 `4260fafc420e913b4df73d48b7f0b7f0bc1d3d6f`
...
- P117a 当前只推进现有三音旋律听写的屏幕钢琴真实 Activity 答案输入：完整播放隐藏旋律后才允许明确开始接收屏幕琴键，三个有序 `note-on` 形成 `piano` answer，再由用户主动检查。
- 参考播放、谱面／演奏回放、压力测试、未开始时的琴键、Web MIDI、USB MIDI 与 BLE MIDI 都不能写入 P117a 答案；重播、停止、全局停止、后台、换题、重置、卸载和迟到事件必须 fail closed。
- P117a 不包含五线谱或简谱答案，它们分别保留给 P117b / P117c；也不包含旋律回唱、旋律视唱、麦克风、实时曲线、提交后分析、正式评分、上传、账号、云端或数据库。
- P117a 当前是 implementation candidate，不是已交付能力。QA level recommendation 为 strict；Android 真机触控、扬声器、音频焦点与后台恢复、教师审核和目标用户可用性均尚未形成证据，不能由源码、自动测试、CI 或 APK 构建替代。
