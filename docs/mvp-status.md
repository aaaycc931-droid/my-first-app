...
- 最新已合并音频／生命周期可靠性链：PR #535–#544 已把会话 MediaRecorder、实时音高
  controller、中断失败关闭、钢琴输入、节拍器 pending start、`/practice` 本地录音、目标
  播放、录音分析和本地旋律参考音频解码分别收紧到 stale-safe ports／controllers；本分支
  普通／谱面节奏练习已共用独立 runtime controller；本分支继续让点击延迟校准以第二个
  隔离实例复用同一 scheduler／timer／clock／tap 协议。所有真实 codec、浏览器、Android
