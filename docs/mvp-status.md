...
- 最新已合并练声产品／UI 边界：PR #533，合并提交
  `62a4cdf6eb032835c18306cc2da366d482f538ca`。本机练声保存明确区分“仅保存当前曲线”与
  用户即时选择的“保存当前曲线和录音”，失败时保留当前录音、曲线与既有记录。
- 最新已合并录音回放 UI／runtime 边界：PR #534，合并提交
...
- 最新已合并音频／生命周期可靠性链：PR #535–#547 已把会话 MediaRecorder、实时音高
  controller、中断失败关闭、钢琴输入、节拍器 pending start、`/practice` 本地录音、目标
  播放、录音分析、本地旋律参考音频解码、普通／谱面节奏、点击延迟校准与 recognition
  音符播放与钢琴回放分别收紧到 stale-safe ports／controllers；本分支继续收紧临时乐谱
  短参考音／旋律的 shared-audio、timer 与 target lifecycle ownership。所有真实 codec、浏览器、Android
...
- 当前正确性切片：`/recognize` 主结果与开发态 Audiveris 预览的音符播放改由同一
  ...
  `NOT_EXECUTED`。
- 当前钢琴数据安全切片：本机演奏记录与学习谱面回放的 timer/run/cycle ownership 改由
  latest-wins controller 承接；旧 callback 不再能停止替换播放、复活旧 loop，或在停止
  回放后写入新建的用户演奏 recorder。真实音色、踏板、长循环、WebView／真机与 MIDI
  硬件 QA 仍为 `NOT_EXECUTED`。
- 当前临时乐谱参考播放可靠性切片：短参考音／参考旋律改由 latest-wins controller 与共享
  browser audio channel 承接；旧旋律 completion 不再能停止替换播放，全局停止与同 id
  target 失效会同步清 timer/source/UI。真实浏览器音频、WebView／真机、后台／锁屏与音频
  焦点 QA 仍为 `NOT_EXECUTED`。
...
- 当前密码恢复候选：未登录入口发送防枚举恢复邮件，只有带有效 session 的 `PASSWORD_RECOVERY` 事件进入专用新密码界面；`USER_UPDATED` 先于更新 promise 的顺序已受保护。生产 Redirect URL allowlist、真实恢复邮件、过期链接、浏览器／WebView／真机矩阵仍为 `NOT_EXECUTED`
- 最新已合并 Web 私人练习历史切片：PR #527 内容已进入 `main`（内容提交 `a162f2e45127a1fcf54f9eb24603cabe95cf2da5`）。登录用户可在账户页读取当前 owner 最近 20 条已完成且已发布的系统课程核对记录，并从记录回到同一练习；读取显式 owner-bound、双层验证课程来源与非评分白名单，支持空态、失败重试和换用户迟到结果屏蔽。生产 RLS／真实多账户、跨浏览器、可访问性与目标用户 QA 仍为 `NOT_EXECUTED`
...
- 本机练声保存区已把持久化介质明确分为“仅保存当前曲线”和用户即时选择的
  ...
  仍为 `NOT_EXECUTED`。
- 当前会话录音、已保存练声录音和 P113 逐音片段的 Blob 回放已统一经过平台无关
  latest-wins controller 与浏览器 adapter；共享 UI 不再直接创建 `Audio` 或 object URL。
  A→B 快速切换后，A 的迟到 play 结果、ended／error 回调和 duration timer 均不能停止 B、
  覆盖 B 状态或显示陈旧错误，URL 由 adapter 幂等单次释放。该切片不改变 MediaRecorder、
