...
- 最新已合并产品功能基线：P118c / PR #409，合并提交 `55093e4d1004bc97408f45ef0e2a26a2adee6c0b`
...
- P118d 可解释非评分复练推荐已形成独立 implementation candidate，固定复用现有 MRU 复练队列首项并显示来源事实；它不读取 `outcome`、画像计数或 P118b 统计，存储异常时失败关闭。远端 PR、CI、review 与合并仍待执行。
- P118e 只有在 P118d 的代码、文档、CI、review 与合并全部闭环后才可成为下一独立教学切片；当前不得提前实现。
