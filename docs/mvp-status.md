...
- 当前 Web 私人练习历史候选：登录用户可在账户页读取当前 owner 最近 20 条已完成系统课程核对记录，并从记录回到同一练习；读取显式 owner-bound、只消费非评分白名单字段，支持空态、失败重试和换用户迟到结果屏蔽。生产 RLS／真实多账户、跨浏览器、可访问性与目标用户 QA 仍为 `NOT_EXECUTED`
