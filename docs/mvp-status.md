...
- 2026-06-20: Added browser local-only Melody Attempt History for the Melody Step Practice Flow. Successful local pitch attempts can be listed against melody steps for review within the current browser session only; this does not upload audio, persist history to a server, add formal scoring, add rhythm evaluation, call AI APIs, change `/api/recognize`, change the recognition provider union, add PDF upload, connect Audiveris, or change the pitch estimator / benchmark gates.
