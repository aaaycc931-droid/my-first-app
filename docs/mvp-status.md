...
- 最新已合并产品功能基线：S3 MusicXML/MXL 自然音根音五类受控和弦标记 / PR #468，合并提交 `689448942135dd6e8155edbf4797b51a0fe488d5`
...
- 导入边界见 `docs/s3-local-score-project-musicxml-import-acceptance.md`，导出边界见 `docs/s3-local-score-project-musicxml-export-acceptance.md`；单附点、单段歌词、单指法、单音演奏法、单事件力度记号、单事件制音踏板记号、受控和弦标记、fermata、圆滑线和延音线双向边界分别见对应的 S3 round-trip acceptance 文档，其中和弦切片见 `docs/s3-local-score-project-musicxml-chord-symbol-round-trip-acceptance.md`。浏览器真实导入／下载／重开、Android WebView／真机、第三方独立阅读器、和弦／踏板／力度／演奏法／指法显示与真实播放、升降根音／转位／其他和弦类别、左右手／替代指法、歌词排版／多 verse／melisma、真实音频与歌唱对齐、教师审核、MIDI、OMR、完整 MusicXML、S3 与正式版 V1 均仍为 `NOT_EXECUTED` 或未完成。
