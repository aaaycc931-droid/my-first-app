...
- 最新产品功能候选：P118a 中文本地课程路径与课程进度；PR、合并提交、Actions 与 Android 工件只在 GitHub 形成对应事实后补录
- 最近已合并产品功能基线：P117e / PR #399，合并提交 `c2fc6a8943c9c432c850f3d0c89455b378c2cdd5`
- 最新仓库维护：PR #400，完成 P117 合并后状态收尾并将教学主轴切换至 P118
...
- P118a 当前形成 implementation candidate：安装包内版本化中文课程包含两章四课，稳定绑定既有单音、音程、节奏与旋律听写 Activity；只有从当前已解锁课节入口产生且已主动核对的匹配结果才能记录本机完成事实并顺序解锁。
- P118a 只消费现有 Activity 结果，不创建平行答案或能力协议；本机 envelope 只保存课程／内容版本、已完成课节 ID 和修订号，旧版本显式迁移，损坏、错配与陈旧归属 fail closed，并可独立二次确认重置。
- P118a 不生成正式能力评级，不上传课程数据，不新增账户、云端、数据库、网络依赖或 `INTERNET` 权限。P118a 合并后 next slice 是 P118b 本地详细练习统计；P118c 的复练队列、P118d 的推荐和 P118e 的整合继续拆分。
