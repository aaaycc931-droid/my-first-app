...
2. 节奏回模（P116b，当前切片）：本地播放后复现，陈旧 attempt、重播、暂停、后台与重置必须 fail closed。
3. 节奏找错（P116c，下一切片）：只标记题目中可验证的漏拍、拆分、合并或位移，不推断演奏能力。
4. 节奏听写（P116d）：复用已确认的谱面草稿协议，必须经过预览、检查、修改／确认后才能进入练习。
