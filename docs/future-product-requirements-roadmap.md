...
- P118a（当前 implementation candidate）：安装包内简体中文课程冻结稳定、版本化的课程／章节／课节／目标／Activity 映射，并用只含版本、已完成课节 ID 和修订号的本机进度执行顺序解锁；只有从当前课节入口产生且已核对的匹配 Activity 结果才能完成课节。旧版本显式迁移，损坏、错配和陈旧归属 fail closed；不产生正式能力评级，不上传课程数据，不新增账户、云端或数据库。
- P118b（P118a 合并后的 next slice）：按活动类型、题目族与时间窗口聚合最小必要、非敏感、非音频事实，并提供可解释的本地详细练习统计；不得保存录音、PCM、Blob、原始音高帧或生物特征，也不得包装成专业能力结论。
- P118c–P118e：随后分别推进薄弱点复练队列、可解释非评分推荐，以及课程／统计／复练／推荐整合；每项保持独立 PR。
