...
- P118d（implementation candidate，远端门禁待执行）：只以建议开关和既有 MRU 复练队列为真源，解释队列第 1 项、同题目族与全部待复练数；不读取 `outcome`、画像计数或统计，不创建平行协议。验收见 `docs/p118d-explainable-non-scoring-recommendation-acceptance.md`。
- P118e：仅在 P118d 的 PR、CI、review 和合并闭环后推进课程／统计／复练／推荐整合，保持独立 PR。
