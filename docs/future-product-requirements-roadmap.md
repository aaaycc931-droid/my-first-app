...
- P118e（当前唯一教学 next slice，implementation candidate 实现中）：用中文“本机学习总览”分别组合既有课程核对数、当前保留的练习动作事实、精确复练队列和可解释建议。四类来源独立失败关闭，不跨来源推导、不新增协议或持久化；合并前不进入 P119。验收见 `docs/p118e-local-learning-overview-acceptance.md`。
