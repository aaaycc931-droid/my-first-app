...
- P118c（当前 implementation candidate）：复用既有本机复练协议，按题目族分组最多 12 个精确未解决目标并保持 MRU 顺序；不读取 `outcome`，不按正确率或能力排序，不新增存储/schema，不改写 P118b 统计。验收见 `docs/p118c-weak-point-review-queue-acceptance.md`。
- P118d–P118e：随后分别推进可解释非评分推荐，以及课程／统计／复练／推荐整合；每项保持独立 PR。
