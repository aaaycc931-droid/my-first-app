...
- P118a（当前 implementation candidate）：中文课程路径和课程进度模型已形成最小实现，冻结稳定、版本化课程 ID、章节／课节／目标／Activity 映射、本地进度、完成条件、解锁规则与内容版本 fail-closed 边界；合并与 CI 状态以对应 PR 为准，外部人工证据仍为 `NOT_EXECUTED`。
- P118b–P118e：随后分别推进本地详细统计、薄弱点复练队列、可解释非评分推荐，以及课程／统计／复练／推荐整合；每项保持独立 PR。
