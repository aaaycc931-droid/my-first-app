# S3 本机谱项目 MusicXML/MXL 小十三和弦 round-trip 验收
...
- canonical `Cm13` 与 MusicXML 4.0 `<kind>minor-13th</kind>` exact 双向映射；
  `C` 可替换为 `A–G`、`A#–G#` 或 `Ab–Gb`。
- MusicXML 4.0 将 `minor-13th` 定义为 minor-11th 加 major 13th；本切片只交换
  kind 标识，不推断和弦音、voicing、持续区间、播放或伴奏。
- 不修改 canonical schema、storage version、事件 ID、编辑器、谱面显示或用户行为。
- 受控和弦矩阵因此从二十三种扩为二十四种 kind；21 个根音拼写保持不变，共
  504 个组合。
...
- 仅接受无属性、无 namespace、直接位于 measure 下并紧邻目标 note/rest 的
  `<harmony><root>…</root><kind>minor-13th</kind><staff>1</staff></harmony>`；
...
- `Cmin13`、`Cminor13`、`C-13`、`Cm131`、`Cm7add13`、`Cm11add13`、`Cm13/E`、
  `Cm13#5`、`Cm13b5` 均不归一化，导出以 `unsupported-chord-symbol` 阻断。
- add13、add11、degree、别名、转位和 altered 13th 是不同语义，仍保持 fail
  closed，不属于本切片。
- 任一 blocking import 输入必须在分配 canonical event ID 前停止。

## 自动验收

- canonical parser/renderer 覆盖二十四种 kind × 21 个根音拼写 = 504 个组合。
- `C#m13` 与 `Dbm13` 在 `.musicxml` 和 `.mxl` 的 note/rest round-trip 中 exact
