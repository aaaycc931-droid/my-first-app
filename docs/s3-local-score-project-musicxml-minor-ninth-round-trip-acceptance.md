# S3 MusicXML／MXL 小九和弦严格 round-trip 验收
...
- `Cadd9`、`C69`、`Cm7add9`、`Cm9/E`、`Cm9#5`、`Cm9b5`、`Cminor9` 或其他
  alias／扩展／改变音不得归一化为 `Cm9`。
- `Cm9`／`minor-ninth` 不得与 dominant-ninth、major-ninth、minor-seventh、
...
- 纯映射覆盖 minor-ninth、全部 7 个自然根音及其单升／单降形式。
- XML 与 MXL 都以 pitched note 和 rest 上的单升／单降根音验证 exact import；独立
  导出 fixture 验证 `<kind>minor-ninth</kind>`、确定性 XML/MXL、
  pitched note/rest 与既有记号共存，以及 exact re-import。
- `Cadd9`／`C69`／`Cm7add9`／`Cm9/E`／`Cminor9`、altered degree、未知 kind、
  bass／inversion、namespace／大小写、属性、错序、重复和其他既有 blocker 保持
