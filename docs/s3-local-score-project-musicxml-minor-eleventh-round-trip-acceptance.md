# S3 MusicXML／MXL 小十一和弦严格 round-trip 验收
...
- `Cadd11`、`Cm7add11`、`Cm9add11`、`Cmin11`、`Cminor11`、`C-11`、
  `Cm11/E`、`Cm11#5`、`Cm11b5` 或其他 alias／扩展／改变音不得归一化为 `Cm11`。
- `Cm11`／`minor-11th` 不得与 dominant-11th、major-11th、minor-ninth、
  minor-seventh 或其他 kind 混同、降级或推导。
- `<degree>` 及任何 altered degree，slash／bass／inversion、双升降、Unicode
  升降号、属性、额外节点、错序、重复、namespace／大小写变体及非规范间隔继续
  blocking。
- add11、thirteenth 及其他未列出的和弦类别继续 blocking。
- blocking 输入不得分配 canonical event ID，也不得静默删除、截断或改变和弦语义。

## 自动验收

- 纯映射覆盖 minor-11th、全部 7 个自然根音及其单升／单降形式；完整受控矩阵为
  21 类 × 21 个根音 = 441 个组合。
- XML 与 MXL 都以 pitched note 和 rest 上的单升／单降根音验证 exact import；独立
  导出 fixture 验证 `<kind>minor-11th</kind>`、确定性 XML/MXL、
  pitched note/rest 与既有记号共存，以及 exact re-import。
- alias／add11／slash、altered degree、未支持 kind、bass／inversion、namespace／
  大小写、属性、错序、重复和其他既有 blocker 保持失败关闭；blocking import
  不分配 event ID。
