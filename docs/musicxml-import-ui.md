...
2. 使用 Audiveris GUI 的导出功能生成 `.mxl`。具体菜单名称可能随 Audiveris 版本
   不同，请以本机 GUI 为准。
3. dev-only import 现在可以直接选择 `.mxl`；浏览器端不会解压，文件会原样上传到
   `/api/dev/recognize-musicxml`，只在服务端内存中解压并验证内部 MusicXML。
4. 仍可直接选择 Audiveris 导出的 `.musicxml` 或 `.xml` 文件。

仓库不会自动调用 Audiveris CLI 或 Java，`.mxl` 支持也不代表图片识别主流程接入 OMR。

## 在网页中导入

1. 使用上面的两个环境变量启动 Next.js 开发服务器。
2. 打开首页，找到 `Experimental: Import MusicXML` 区域。
3. 选择 `.musicxml`、`.xml` 或 `.mxl` 文件。
4. 点击“导入并验证播放”。
...
当前 dev-only 入口只接受 `.musicxml`、`.xml` 和 `.mxl`，上传文件大小上限仍为 2 MB。`.mxl` 解压后的内部 MusicXML 文本最大支持 4 MB，zip entry 最多 100 个。空文件、超过限制的文件、路径穿越 rootfile、缺失或异常的 `META-INF/container.xml`、无法找到 `<score-partwise>` / `<score-timewise>` 的 XML 都会被安全拒绝，并在 MusicXML 导入区域显示友好错误。

`.mxl` 解压只发生在 `/api/dev/recognize-musicxml` 服务端内存中，不写磁盘，不进入浏览器端，也不会影响原图片上传流程或图片上传的 10 MB 限制。未选择文件、正在导入或选择了不支持的文件时，导入按钮不可用；解析或 API 调用失败时，API 返回的友好错误会显示在 MusicXML 导入区域。

## QA 与静态验证

Phase A16 的完整手动验证步骤见
[`docs/musicxml-import-ui-qa.md`](./musicxml-import-ui-qa.md)，覆盖默认隐藏、开关开启、
`.musicxml` / `.xml` 导入、`.mxl` dev-only 导入、API 未开启错误和图片上传回归。
...
不要提交本地生成的 PDF、MXL、XML、OMR、log、图片或真实乐谱样本文件。`.mxl` 验证数据应由脚本动态生成。
