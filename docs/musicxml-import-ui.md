...
当前 dev-only 入口只接受 `.musicxml` 和 `.xml`，文件大小上限为 2 MB。空文件、超过
2 MB 的文件和其他扩展名都会被安全拒绝，并在 MusicXML 导入区域显示友好错误。

如果选择 `.mxl`，页面会明确提示先将其改名为 `.zip` 并人工解压出内部 XML 文件，不会
尝试上传或解压该文件。`.mxl` 仍然不受支持，仓库也没有实现 `.mxl` 解压。未选择文件、
正在导入或选择了不支持的文件时，导入按钮不可用；解析或 API 调用失败时，API 返回的
友好错误会显示在 MusicXML 导入区域，不影响原图片上传流程或图片上传的 10 MB 限制。
