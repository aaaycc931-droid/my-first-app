# S3 MusicXML／MXL 大九和弦严格 round-trip 验收
...
- `Cm9`、`Cadd9`、`C69`、`Cmaj7add9`、`Cmaj9/E`、`Cmaj9#5`、`Cmaj9b5`、
  `Cmajor9` 或其他 alias／扩展／改变音不得归一化为 `Cmaj9`。
- `Cmaj9`／`major-ninth` 不得与 dominant-ninth、major-seventh、minor-seventh、
  major、minor 或其他 kind 混同、降级或推导。
...
- 纯映射覆盖 major-ninth、全部 7 个自然根音及其单升／单降形式。
- XML 与 MXL 都以 pitched note 和 rest 上的单升／单降根音验证 exact import；独立
  导出 fixture 验证 `<kind>major-ninth</kind>`、确定性 XML/MXL、
  pitched note/rest 与既有记号共存，以及 exact re-import。
- `Cm9`／`Cadd9`／`C69`／`Cmaj7add9`／`Cmaj9/E`／`Cmajor9`、altered degree、
  未知 kind、bass／inversion、namespace／大小写、属性、错序、重复和其他既有
