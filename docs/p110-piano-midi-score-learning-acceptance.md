...
2. 用户主动请求 Web MIDI 权限后，可选择输入设备，将 note-on、note-off、0–127 力度和 CC64 延音踏板映射到本地钢琴。设备切换、拔线、主动断开、切页、后台和卸载必须全停；迟到的权限成功或失败结果不得重新绑定输入或覆盖当前状态。
...
6. MusicXML 草稿只有在用户主动检查并确认后才允许播放；任何修改都会使确认失效。替换、错误或清除文件必须清除旧草稿和下游可播放状态；旧文件迟到的读取成功或失败不得在清除、替换或卸载后复活草稿或错误。
...
- 真实挂载移动 React 测试覆盖六音色 UI、不支持 MIDI 降级、设备力度/踏板/断连、迟到 MIDI 权限、MusicXML 检查确认修改清除及迟到读取，以及原创谱面/瀑布播放。
