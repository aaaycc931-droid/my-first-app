...
The P4a exploratory observations should be read as diagnostic only. In the recorded run, frequency drift retained fewer dominant-cluster frames and a narrower frame min/max range, while vibrato still showed the same aggregate estimate and extreme frame spread because its candidate frames did not form a sufficiently dominant rounded-MIDI cluster. This means P4a is a small regression-safe aggregation hardening step, not a complete vibrato estimator solution.
