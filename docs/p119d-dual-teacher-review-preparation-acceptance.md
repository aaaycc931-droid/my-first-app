...
状态：**PRE-REVIEW PREPARATION CANDIDATE / sample NOT APPROVED / teacher review NOT_EXECUTED**
...
本阶段的 153 个 item ID 明确是 **候选**，不是最终批准结果。两名真实教师尚未确认样本量、分层或具体 item ID，因此：

- `approvedBeforeReview` 必须保持 `false`；
- `teacherReviews` 必须保持空；
- 不得把候选 ID 写入 `evidence.local.json` 并改成 `COMPLETE`；
- 不得开始正式审核、正式样本、正式评估或声明教育通过；
- `npm run validate:p119-formal-stage-gate` 应继续以退出码 2 表示外部证据阻塞。
...
seed 绑定生成候选前已存在的 main commit 与冻结 manifest 摘要，减少事后挑选空间。该方法仍不是教师批准；教师可要求新规则和新 seed，但必须先记录理由并产生新版本文件，不能静默覆盖 v1。
...
当前没有 `evidence.local.json` 的真实批准与教师记录，预期结果是 `BLOCKED` / exit 2。

## 7. 明确未执行

- 两位教师对样本量、分层与具体 ID 的批准；
- 教师身份、资质、独立性与签署核验；
- 逐题六维审核；
- 教育有效性、总体正确率或发布结论；
- 专业路线每档 40；
