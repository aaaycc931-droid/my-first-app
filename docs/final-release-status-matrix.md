...
| V1-04 | 系统课程与题库 | IN_PROGRESS | 基础课程；单音、音程、节奏持久化；P115a–P115i 已合并，覆盖和弦/转位、和声进行、音阶/调式、七和弦、排列、调制、统一自定义器、音程比较/非评分模唱和声部进行线索 | P116–P118 节奏、旋律与课程纵向闭环；双教师题量/难度/解释校准；Android 真实用户验证 |
...
| V1-23 | 发布证据包 | IN_PROGRESS | P115i PR #388 / run `29736262953` 的 quality、Android API 36 构建、独立 APK 复核与 artifact `8458458938` 已核对；状态清理合并后 run `29740400363` 与 artifact `8460154570` 再次通过 | P116–P118 功能冻结、专用签名与 APK 独立摘要证据、P104 真实数据/设备/教育证据、删除/恢复与用户验收证据 |
| V1-24 | Android 本地 APK | IN_PROGRESS | 离线题库、实时曲线/录音/目标/观察、本机记录/复练、P106–P114m 共享能力；P115a–P115i 已合并并通过 `android-local` Debug APK 门禁 | P116–P118 功能完整候选、P104 三档真机/真实人声、专用签名、升级与前向回滚 |
...
产品所有者已明确恢复开发；P114a–P114m 与 P115a–P115i 已合并。下一教学主轴进入 P116 节奏专业套件；角色设定不代表伙伴运行时，合并代码和自动测试也不代表真机、真实人声或教育证据。当前依赖顺序如下：
