...
| V1-04 | 系统课程与题库 | IN_PROGRESS | 基础课程；单音、音程、节奏持久化；P115a 和弦/转位、P115b 和声进行与 P115c 音阶/调式已合并；P115d 候选含七和弦基础 48、进阶 96、挑战 192 个稳定变体 | P115d 严格本地验证、PR CI 与审查；调制、开放/密集排列、完整定制、基础视唱、课程三难度与进度、教育审核；Android 难度仍待审核校准 |
...
| V1-12 | 学习历史与复习 | IN_PROGRESS | 私有 practice attempts、课程答案摘要；Android 本机最小错题队列；P114m 非评分画像已合并；P115a–P115c 已扩展和弦、进行、音阶/调式事实，P115d 候选拟扩展七和弦事实；用户主动保存的练声曲线/可选录音记录 | Web 历史 UI、正式技能画像、跨设备同步；P115d 迁移回归与 Android 跨版本/跨重启和配额真机验证；教育审核 |
...
| V1-23 | 发布证据包 | IN_PROGRESS | P115b PR #381 / run `29684947244` 的 quality、Android API 36 构建、独立 APK 复核与 artifact `8441785466` 已核对；ZIP digest `e6b6f8edb464b4159893c8f09023c28cbf4cbfcdaccceb302a8186a088a27ba2`；P115c PR #382 合并点已确认 | P115c 独立 APK 证据补录；P115d 及后续功能冻结、专用签名与 APK 独立摘要证据、P104 真实数据/设备/教育证据、删除/恢复与用户验收证据 |
| V1-24 | Android 本地 APK | IN_PROGRESS | 离线题库、实时曲线/录音/目标/观察、本机记录/复练、P106–P114m 共享能力；P115c 源码已合并，最新已独立复核上传的 Debug APK 证据仍为 P115b | P115c APK 证据补录；P115d–P118 功能完整候选、P104 三档真机/真实人声、专用签名、升级与前向回滚 |
...
产品所有者已明确恢复开发；P114a–P114m 与 P115a–P115c 已合并，P115c 通过 PR #382 squash merge，当前 main 为 `ccc77afa1f0c2c9675d64a6dc6fc221e3df45094`。当前进入 P115d 七和弦性质与转位听辨的本地严格验证阶段；角色设定不代表伙伴运行时，候选代码和本地测试也不代表 PR CI、真机或教育证据。当前依赖顺序如下：
...
12. P115a–P115c 已分别通过 PR #380/#381/#382 合并；P115d 当前是本地严格验证候选：新增四类七和弦的三难度性质/转位题库、统一非评分答案、复练与学习事实闭环；PR CI、Android 真机、教育审核、调制和完整定制器仍未完成。
