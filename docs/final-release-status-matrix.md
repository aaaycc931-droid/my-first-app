...
| V1-04 | 系统课程与题库 | IN_PROGRESS | 基础课程；单音、音程、节奏持久化；P115a 和弦/转位已合并；P115b 和声进行候选含基础 8、进阶 24、挑战 42 个稳定变体 | 七和弦、调制、音阶/调式、基础/进阶长期题量、基础视唱、课程三难度与进度、教育审核；Android 难度仍待审核校准 |
...
| V1-12 | 学习历史与复习 | IN_PROGRESS | 私有 practice attempts、课程答案摘要；Android 本机最小错题队列；P114m 非评分画像已合并；P115a 已扩展和弦/转位事实，P115b 候选扩展和声进行事实；用户主动保存的练声曲线/可选录音记录 | Web 历史 UI、正式技能画像、跨设备同步；Android 跨版本/跨重启和配额真机验证；教育审核 |
...
| V1-23 | 发布证据包 | IN_PROGRESS | P115a PR #380 / run `29682428365` 的 quality、Android API 36 构建、独立 APK 复核与 artifact `8441012329` 已核对；ZIP digest `7d701b97f04f8f528faed9cc50b6f11f372a7349b19acef0cb8eada849f39fba` | P115b 及后续功能冻结、专用签名与 APK 独立摘要证据、P104 真实数据/设备/教育证据、删除/恢复与用户验收证据 |
| V1-24 | Android 本地 APK | IN_PROGRESS | 离线题库、实时曲线/录音/目标/观察、本机记录/复练、P106–P114m 共享能力；P115a Debug APK 已构建、复核并上传 | P115b–P118 功能完整候选、P104 三档真机/真实人声、专用签名、升级与前向回滚 |
...
产品所有者已明确恢复开发；P114a–P114m 与 P115a 已合并，P115a 通过 PR #380 squash merge，当前 main 为 `a42ff681278816ab3cd277a4a6e403dc77f457be`。当前进入 P115b 和声进行与终止式听辨 implementation candidate；角色设定不代表伙伴运行时，MIDI 合并也不代表真机证据。当前依赖顺序如下：
...
12. P115a 已通过 PR #380 合并；P115b 当前是 ACTIVE implementation candidate：新增和声进行与终止式三难度题库、逐和弦播放、统一答案、复练与学习事实闭环；七和弦、调制、音阶/调式、完整题量和教育审核仍未完成。
