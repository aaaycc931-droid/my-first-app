...
| V1-18 | 浏览器与真实设备 | IN_PROGRESS | 多次桌面 Preview QA；历史移动端 magic-link/播放 QA | 固定矩阵、2 台 iOS + 3 台 Android、麦克风/录音全流程 |
...
| V1-24 | 最终生产发布 | BLOCKED | 当前 P84 生产版本已部署 | V1-02 至 V1-23 的全部 MUST 门槛尚未通过 |
...
| APK / 原生 App | OUT_OF_SCOPE | V1 是响应式 Web 产品 |
...
1. V1-03 数据导出与删除：先闭合已经上线的账户/数据库责任。
2. V1-12 学习历史与复习：把已有 attempts 转化为可见学习闭环。
3. V1-04 系统课程：补齐视唱、题量、难度与课程进度。
4. V1-13 / V1-14 算法基准：冻结数据集和量化证据。
5. V1-07 钢琴辅助：建立练习页与独立页最小正式闭环。
6. V1-08 至 V1-11 私有素材与 worker：在删除、权限和运行手册基础上接入。
7. V1-16 至 V1-23 完成发布工程、真实设备、教育和用户验收。
