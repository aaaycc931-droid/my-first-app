...
| V1-23 | 发布证据包 | IN_PROGRESS | P95 run `29496649623` 两个 job PASS，artifact `8374699870` 的内部 APK SHA-256 与 ZIP digest 已核对，C1 已 PASS | P96 CI；汇总全部基准、设备、删除/恢复、教育和用户验收证据 |
| V1-24 | Android 本地 APK | IN_PROGRESS | Capacitor 离线工程、四题型三难度版本化题库、可复现随机题序、本机错题复练与 v1→v2 迁移、离线参考钢琴、无网络权限；P95 artifact `8374699870` 内 APK SHA-256 为 `9a902fd2e6dfa2acd02665d6a74f8729e1d3deaa453f9860a4ebe256f7eedd93`。 | P96 CI；首台真机安装、跨版本/跨重启复练、题库听感与难度、钢琴多指/延音、音频、后台恢复与存储异常；三档设备；专用 release 签名、升级与回滚 |
...
1. V1-24 Android 本地 APK：完成 P96 三难度题库 CI 后进入 P97 本地麦克风与实时音高引擎；同时准备首台真机安装、四类练习音频、钢琴多指/延音、本机错题队列跨版本/跨重启/清除、后台恢复、存储异常和断网回归；当前不做 AAB、商店、TWA、域名关联或认证回跳。
