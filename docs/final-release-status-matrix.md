...
产品所有者已要求完整交付 P114b 后停止，当前正在将四类现有练习汇合到统一活动协议。当前依赖顺序如下：
...
4. P114b 将音程、节奏和三音旋律听写迁入同一协议，并保持旧题库、课程 RPC、Web Audio 和复练 v2 兼容；合并后按产品要求暂停，不自动进入 P114c。这仍不是完整 P114，五线谱、唱名、点击、麦克风、MIDI 等答案适配器尚未交付。
5. P115–P118 教学泳道与 S（制谱/OMR）、A（伴奏/歌曲）、必要 C（云端/内容/教师）泳道按依赖滚动；钢琴 P106–P110 只补真实体验和证据缺口。
6. P119/Q 必须汇合算法、钢琴、教学、制谱、伴奏、云服务、教育和目标用户验收；P120 只是八产品范围明确的专业私测候选，不是公开最终版。
7. V1-03 / V1-02 / V1-15 Web 数据生命周期继续是正式 Web 责任；最终产品同时建设本地与中国区合规云端能力，当前离线优先不是终局。
