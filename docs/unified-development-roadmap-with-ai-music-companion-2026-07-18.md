...
当前执行状态：**P117a–P117e MERGED / NEXT P118a CHINESE COURSE PATH AND PROGRESS MODEL**

适用项目：`aaaycc931-droid/my-first-app`

## 1. 当前事实与本次决策

本节保留路线形成时从 P106 到 P115 的逐项 provenance；当前远端权威基线已更新为 P117e / PR #399，合并提交 `c2fc6a8943c9c432c850f3d0c89455b378c2cdd5`：
...
- P116a–P116d 已通过 PR #391–#394 合并，覆盖节奏视读、回模、找错与听写；P117a–P117e 已通过 PR #395–#399 合并，覆盖旋律听写多输入、三音回唱与可见目标视唱 implementation candidate。

本次产品决策是：

> AI 音乐伙伴正式进入全能音乐学习系统的长期产品终局和开发主路线，但伙伴不能先于可信音乐能力成为装饰性聊天壳。P114 共享协议以及 P115–P117 教学切片已经合并，当前教学主轴进入 P118a；首发伙伴仍须在可信证据和活动状态机可用后作为完整 vertical slice 进入。
...
该暂停点已由产品所有者在 2026-07-18 明确解除；截至 2026-07-22，P114a–P114m、P115a–P115i、P116a–P116d 与 P117a–P117e 已合并，P114i 角色设定没有伙伴运行时。当前教学主轴进入 P118a 中文课程路径和课程进度模型；BLE/USB 真机、真实麦克风、真实人声和后续发布门槛仍待独立完成，共享运行时交付不等于伙伴、设备或教育证据完成。
...
3. 当前按 `future-product-requirements-roadmap.md` 从 P118a 中文课程路径和课程进度模型开始，先冻结稳定版本化的课程／章节／课节 ID、Activity 映射、本地进度、完成／解锁规则与安全迁移；
4. 不先做伙伴形象、首页悬浮物或自由聊天；
5. P118b–P118e 的本地详细统计、薄弱点复练、可解释非评分推荐和整合继续拆成后续可验证切片；M1a 单伙伴可信反馈闭环仍按证据和依赖门槛启动；
...
本节保留本路线最初建立时的 docs-only 边界；它不是当前执行状态。当前执行状态以文件顶部、`docs/mvp-status.md` 与 `docs/final-release-status-matrix.md` 为准，但以下原则继续有效：
