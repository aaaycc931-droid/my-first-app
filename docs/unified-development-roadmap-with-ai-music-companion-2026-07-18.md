...
- S1/S2 canonical tie／fermata／slur／单附点／单段歌词／单指法／单音演奏法与 S3 受控单 part／staff／voice MusicXML/MXL 导入、导出及仓库内部 round-trip 已形成连续主线；当前严格子集已纳入 note/rest 单附点与 fermata、pitched note 的规范化唯一 `<lyric><text>`、`1–5` 唯一 `<technical><fingering>` 和 accent／staccato／tenuto 唯一 `<articulations>`、相邻时间连续 note 的 slur，以及相邻同音高时间连续 note 的 tie start／stop、跨小节、链式关系和 notations 共存。浏览器真实导入／下载／重开、Android WebView／真机、MuseScore 等第三方独立阅读器、演奏法显示／真实播放、指法显示／左右手／替代指法、歌词排版／多 verse／melisma、真实音频与歌唱对齐、教师审核、MIDI、OMR、完整 MusicXML、S3 与正式版 V1 仍为 `NOT_EXECUTED` 或未完成。
