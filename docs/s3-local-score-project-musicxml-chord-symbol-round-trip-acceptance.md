...
- 本切片只接受根音 `A–G`，以及可选的单个 ASCII `#`／`b`，与十八种无转位和弦：
  ...
  `Cmaj9`（major-ninth）、`Cm9`（minor-ninth）、
...
- canonical parser/renderer 纯映射覆盖十八种 kind、全部 7 个自然根音及其单升／
  单降形式（378 个组合）；XML 与 MXL 以 note/rest 上的升降根音代表组合验证
