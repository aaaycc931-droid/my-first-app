...
- 本切片只接受根音 `A–G`，以及可选的单个 ASCII `#`／`b`，与二十二种无转位和弦：
...
- canonical parser/renderer 纯映射覆盖二十二种 kind、全部 7 个自然根音及其单升／
  单降形式（462 个组合）；XML 与 MXL 以 note/rest 上的升降根音代表组合验证
...
属十三和弦新增严格边界见
`docs/s3-local-score-project-musicxml-dominant-thirteenth-round-trip-acceptance.md`。
