...
- canonical parser/renderer 纯映射覆盖八种 kind、全部 7 个自然根音及其单升／
  单降形式（168 个组合）；XML 与 MXL 以 note/rest 上的升降根音代表组合验证
