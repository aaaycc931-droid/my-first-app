...
- 本切片只接受根音 `A–G`，以及可选的单个 ASCII `#`／`b`，与十五种无转位和弦：
  ...
  `Csus4`（suspended-fourth）、`C5`（power）、
...
- canonical parser/renderer 纯映射覆盖十五种 kind、全部 7 个自然根音及其单升／
  单降形式（315 个组合）；XML 与 MXL 以 note/rest 上的升降根音代表组合验证
