...
assert.deepEqual(loadMobileCourseProgress(storage), { progress, notice: null, sourceStatus: "available" });
...
const invalidCourseLoad = loadMobileCourseProgress(storage);
assert.match(invalidCourseLoad.notice ?? "", /无法读取/);
assert.equal(invalidCourseLoad.sourceStatus, "unavailable");
assert.equal(values.has(MOBILE_COURSE_PROGRESS_STORAGE_KEY), true);
...
assert.equal(loadMobileCourseProgress(throwing).sourceStatus, "unavailable");
