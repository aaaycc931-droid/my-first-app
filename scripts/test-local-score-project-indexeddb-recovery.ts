...
const run = async () => {
  ...
  assert.equal(migratedList[0]?.schemaVersion, "local-score-project-storage-v14");
  assert.equal(migratedList[0]?.document.schemaVersion, "score-document-v13");
  ...
  assert.equal(migratedLoad.project?.schemaVersion, "local-score-project-storage-v14");
  ...
  assert.equal(previousList[0]?.schemaVersion, "local-score-project-storage-v14");
  assert.equal(previousList[0]?.document.schemaVersion, "score-document-v13");
  const previousLoad = await previousStore.get(previousSeed.projectId);
  assert.equal(previousLoad?.schemaVersion, "local-score-project-storage-v14");
  assert.deepEqual(
    await getRawRecord({
  ...
  assert.equal(storageV6List[0]?.document.schemaVersion, "score-document-v13");
  ...
  assert.equal(migratedRaw.schemaVersion, "local-score-project-storage-v14");
  ...
  await putRawRecord({
    ...
    value: {
      ...
      schemaVersion: "local-score-project-storage-v15",
    },
  });
