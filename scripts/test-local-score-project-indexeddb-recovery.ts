...
const run = async () => {
  ...
  assert.equal(migratedList[0]?.schemaVersion, "local-score-project-storage-v8");
  assert.equal(migratedList[0]?.document.schemaVersion, "score-document-v7");
  ...
  assert.equal(migratedLoad.project?.schemaVersion, "local-score-project-storage-v8");
  ...
  assert.equal(previousList[0]?.schemaVersion, "local-score-project-storage-v8");
  assert.equal(previousList[0]?.document.schemaVersion, "score-document-v7");
  const previousLoad = await previousStore.get(previousSeed.projectId);
  assert.equal(previousLoad?.schemaVersion, "local-score-project-storage-v8");
  assert.deepEqual(
    await getRawRecord({
  ...
  assert.equal(storageV6List[0]?.document.schemaVersion, "score-document-v7");
  ...
  assert.equal(migratedRaw.schemaVersion, "local-score-project-storage-v8");
  ...
  await putRawRecord({
    ...
    value: {
      ...
      schemaVersion: "local-score-project-storage-v9",
    },
  });
