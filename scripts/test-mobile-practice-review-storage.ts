...
assert.equal(migratedEnvelope.schemaVersion, 3);
assert.equal(migratedEnvelope.catalogVersion, 3);
...
assert.equal(
  ...
  "a second load of the rewritten v3 queue must not rewrite storage again",
);
