...
assert.equal(migratedEnvelope.schemaVersion, 4);
assert.equal(migratedEnvelope.catalogVersion, 4);
...
assert.equal(
  ...
  "a second load of the rewritten v4 queue must not rewrite storage again",
);
