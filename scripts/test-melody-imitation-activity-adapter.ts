...
const alignmentTargets = getLocalMelodyImitationP113Targets(timeline);

const definition = createMelodyImitationActivityDefinition(timeline);
...
assert.deepEqual(definition.target.checkPolicy.requiredTargetIds, alignmentTargets.map((target) => target.targetId));
...
const binding: MelodyImitationAnalysisBinding = {
  ...
  attemptId: "melody-session:attempt:1",
  playbackQualificationId: "playback-qualification-1",
...
const targetEvidence = (
  ...
  const target = alignmentTargets[index]!;
  ...
  return {
    ...
    state,
    detectedNote: reliable ? target.label : null,
...
const alignment = (states: readonly OfflineTargetEvidence["state"][]): OfflineNoteAlignmentResult => {
  ...
  const reliableCount = evidence.filter((item) => item.state === "close" || item.state === "high" || item.state === "low").length;
  return {
...
) => adaptMelodyImitationActivityEvidence({ definition, timeline, currentBinding, capturedBinding, result });
...
assert.deepEqual(close.evidence.map((item) => item.targetId), alignmentTargets.map((target) => target.targetId));
for (const evidence of close.evidence) {
  ...
  ]) assert.ok(evidence.evidenceId.includes(identity));
}
...
for (const states of [
  ...
  assert.ok(bundle.answer, "complete rejected/missing positions remain explicit evidence references");
  assert.equal(bundle.checkEvidence.assessmentMode, "non-scoring");
}
...
const wrongOrder = alignment(["close", "close", "close"]);
const reordered = { ...wrongOrder, targetEvidence: [wrongOrder.targetEvidence[1]!, wrongOrder.targetEvidence[0]!, wrongOrder.targetEvidence[2]!] };
...
const missingPosition = { ...wrongOrder, targetEvidence: wrongOrder.targetEvidence.slice(0, 2) };
...
const lawfulExtra = {
  ...wrongOrder,
  segments: [...wrongOrder.segments, extraSegment],
  extraSegmentIds: ["extra-segment"],
  summary: {
    ...wrongOrder.summary,
    segmentCount: wrongOrder.summary.segmentCount + 1,
    usableSegmentCount: wrongOrder.summary.usableSegmentCount + 1,
    extraSegmentCount: 1,
  },
};
...
assert.ok(lawfulExtraBundle.answer, "a real extra segment remains current-attempt evidence");
assert.equal(lawfulExtraBundle.evidence.length, 3);

const contradictoryExtra = {
  ...wrongOrder,
  extraSegmentIds: ["extra-segment"],
  summary: { ...wrongOrder.summary, extraSegmentCount: 1 },
};
...
assert.throws(() => createMelodyImitationActivityDefinition({
  ...timeline,
  events: [timeline.events[1]!, timeline.events[0]!, timeline.events[2]!],
}), /position|identity/);
assert.throws(() => createMelodyImitationActivityDefinition({
  ...timeline,
  events: timeline.events.slice(0, 2) as unknown as typeof timeline.events,
}), /three events/);

console.log("P117d melody imitation Activity adapter tests passed.");
