...
const strictDiminishedSeventhHarmonyXml = supportedXml
  .replace(
    "<note><pitch><step>C</step>",
    '<harmony><root><root-step>C</root-step><root-alter>1</root-alter></root><kind>diminished-seventh</kind><staff>1</staff></harmony><note><pitch><step>C</step>',
  )
  .replace(
    "<note><rest/>",
    '<harmony><root><root-step>D</root-step><root-alter>-1</root-alter></root><kind>diminished-seventh</kind><staff>1</staff></harmony><note><rest/>',
  );
for (const sourceFormat of ["musicxml", "mxl"] as const) {
  let diminishedSeventhEventIds = 0;
  const diminishedSeventhDraft = createLocalScoreProjectMusicXmlImportDraft({
    xml: strictDiminishedSeventhHarmonyXml,
    fileName: `严格减七和弦.${sourceFormat}`,
    sourceFormat,
    projectId: `import-project-diminished-seventh-${sourceFormat}`,
    now: "2026-07-30T09:50:00.000Z",
    createEventId: () =>
      `diminished-seventh-${sourceFormat}-${++diminishedSeventhEventIds}`,
  });
  assert.equal(diminishedSeventhDraft.status, "ready");
  assert.deepEqual(diminishedSeventhDraft.issues, []);
  assert.deepEqual(
    diminishedSeventhDraft.project?.document.parts[0].staves[0].voices[0]
      .measures.flatMap((measure) => measure.events)
      .map((event) => event.chordSymbol),
    ["C#dim7", "Dbdim7", null, null, null],
    `${sourceFormat} must preserve diminished-seventh symbols`,
  );
  assert.equal(diminishedSeventhEventIds, 5);
}

const strictHalfDiminishedHarmonyXml = supportedXml
  .replace(
    "<note><pitch><step>C</step>",
    '<harmony><root><root-step>C</root-step><root-alter>1</root-alter></root><kind>half-diminished</kind><staff>1</staff></harmony><note><pitch><step>C</step>',
  )
  .replace(
    "<note><rest/>",
    '<harmony><root><root-step>D</root-step><root-alter>-1</root-alter></root><kind>half-diminished</kind><staff>1</staff></harmony><note><rest/>',
  );
for (const sourceFormat of ["musicxml", "mxl"] as const) {
  let halfDiminishedEventIds = 0;
  const halfDiminishedDraft = createLocalScoreProjectMusicXmlImportDraft({
    xml: strictHalfDiminishedHarmonyXml,
    fileName: `严格半减七和弦.${sourceFormat}`,
    sourceFormat,
    projectId: `import-project-half-diminished-${sourceFormat}`,
    now: "2026-07-30T11:20:00.000Z",
    createEventId: () =>
      `half-diminished-${sourceFormat}-${++halfDiminishedEventIds}`,
  });
  assert.equal(halfDiminishedDraft.status, "ready");
  assert.deepEqual(halfDiminishedDraft.issues, []);
  assert.deepEqual(
    halfDiminishedDraft.project?.document.parts[0].staves[0].voices[0]
      .measures.flatMap((measure) => measure.events)
      .map((event) => event.chordSymbol),
    ["C#m7b5", "Dbm7b5", null, null, null],
    `${sourceFormat} must preserve half-diminished symbols`,
  );
...
const strictPowerChordHarmonyXml = supportedXml
  .replace(
    "<note><pitch><step>C</step>",
    '<harmony><root><root-step>C</root-step><root-alter>1</root-alter></root><kind>power</kind><staff>1</staff></harmony><note><pitch><step>C</step>',
  )
  .replace(
    "<note><rest/>",
    '<harmony><root><root-step>D</root-step><root-alter>-1</root-alter></root><kind>power</kind><staff>1</staff></harmony><note><rest/>',
  );
for (const sourceFormat of ["musicxml", "mxl"] as const) {
  let powerChordEventIds = 0;
  const powerChordDraft = createLocalScoreProjectMusicXmlImportDraft({
    xml: strictPowerChordHarmonyXml,
    fileName: `严格强力和弦.${sourceFormat}`,
    sourceFormat,
    projectId: `import-project-power-chord-${sourceFormat}`,
    now: "2026-07-31T08:00:00.000Z",
    createEventId: () =>
      `power-chord-${sourceFormat}-${++powerChordEventIds}`,
  });
  assert.equal(powerChordDraft.status, "ready");
  assert.deepEqual(powerChordDraft.issues, []);
  assert.deepEqual(
    powerChordDraft.project?.document.parts[0].staves[0].voices[0]
      .measures.flatMap((measure) => measure.events)
      .map((event) => event.chordSymbol),
    ["C#5", "Db5", null, null, null],
    `${sourceFormat} must preserve power chord symbols`,
  );
  assert.equal(powerChordEventIds, 5);
}

const strictDominantNinthHarmonyXml = supportedXml
  .replace(
    "<note><pitch><step>C</step>",
    '<harmony><root><root-step>C</root-step><root-alter>1</root-alter></root><kind>dominant-ninth</kind><staff>1</staff></harmony><note><pitch><step>C</step>',
  )
  .replace(
    "<note><rest/>",
    '<harmony><root><root-step>D</root-step><root-alter>-1</root-alter></root><kind>dominant-ninth</kind><staff>1</staff></harmony><note><rest/>',
  );
for (const sourceFormat of ["musicxml", "mxl"] as const) {
  let dominantNinthEventIds = 0;
  const dominantNinthDraft = createLocalScoreProjectMusicXmlImportDraft({
    xml: strictDominantNinthHarmonyXml,
    fileName: `严格属九和弦.${sourceFormat}`,
    sourceFormat,
    projectId: `import-project-dominant-ninth-${sourceFormat}`,
    now: "2026-07-31T09:30:00.000Z",
    createEventId: () =>
      `dominant-ninth-${sourceFormat}-${++dominantNinthEventIds}`,
  });
  assert.equal(dominantNinthDraft.status, "ready");
  assert.deepEqual(dominantNinthDraft.issues, []);
  assert.deepEqual(
    dominantNinthDraft.project?.document.parts[0].staves[0].voices[0]
      .measures.flatMap((measure) => measure.events)
      .map((event) => event.chordSymbol),
    ["C#9", "Db9", null, null, null],
    `${sourceFormat} must preserve dominant-ninth symbols`,
  );
...
const strictMajorEleventhHarmonyXml = supportedXml
  .replace(
    "<note><pitch><step>C</step>",
    '<harmony><root><root-step>C</root-step><root-alter>1</root-alter></root><kind>major-11th</kind><staff>1</staff></harmony><note><pitch><step>C</step>',
  )
  .replace(
    "<note><rest/>",
    '<harmony><root><root-step>D</root-step><root-alter>-1</root-alter></root><kind>major-11th</kind><staff>1</staff></harmony><note><rest/>',
  );
for (const sourceFormat of ["musicxml", "mxl"] as const) {
  let majorEleventhEventIds = 0;
  const majorEleventhDraft = createLocalScoreProjectMusicXmlImportDraft({
    xml: strictMajorEleventhHarmonyXml,
    fileName: `严格大十一和弦.${sourceFormat}`,
    sourceFormat,
    projectId: `import-project-major-eleventh-${sourceFormat}`,
    now: "2026-08-01T09:00:00.000Z",
    createEventId: () =>
      `major-eleventh-${sourceFormat}-${++majorEleventhEventIds}`,
  });
  assert.equal(majorEleventhDraft.status, "ready");
  assert.deepEqual(majorEleventhDraft.issues, []);
  assert.deepEqual(
    majorEleventhDraft.project?.document.parts[0].staves[0].voices[0]
      .measures.flatMap((measure) => measure.events)
      .map((event) => event.chordSymbol),
    ["C#maj11", "Dbmaj11", null, null, null],
    `${sourceFormat} must preserve major-11th symbols`,
  );
  assert.equal(majorEleventhEventIds, 5);
}

const strictMinorEleventhHarmonyXml = supportedXml
  .replace(
    "<note><pitch><step>C</step>",
    '<harmony><root><root-step>C</root-step><root-alter>1</root-alter></root><kind>minor-11th</kind><staff>1</staff></harmony><note><pitch><step>C</step>',
  )
  .replace(
    "<note><rest/>",
    '<harmony><root><root-step>D</root-step><root-alter>-1</root-alter></root><kind>minor-11th</kind><staff>1</staff></harmony><note><rest/>',
  );
for (const sourceFormat of ["musicxml", "mxl"] as const) {
  let minorEleventhEventIds = 0;
  const minorEleventhDraft = createLocalScoreProjectMusicXmlImportDraft({
    xml: strictMinorEleventhHarmonyXml,
    fileName: `严格小十一和弦.${sourceFormat}`,
    sourceFormat,
    projectId: `import-project-minor-eleventh-${sourceFormat}`,
    now: "2026-08-01T10:00:00.000Z",
    createEventId: () =>
      `minor-eleventh-${sourceFormat}-${++minorEleventhEventIds}`,
  });
  assert.equal(minorEleventhDraft.status, "ready");
  assert.deepEqual(minorEleventhDraft.issues, []);
  assert.deepEqual(
    minorEleventhDraft.project?.document.parts[0].staves[0].voices[0]
      .measures.flatMap((measure) => measure.events)
      .map((event) => event.chordSymbol),
    ["C#m11", "Dbm11", null, null, null],
    `${sourceFormat} must preserve minor-11th symbols`,
  );
  assert.equal(minorEleventhEventIds, 5);
}

const strictDominantThirteenthHarmonyXml = supportedXml
  .replace(
    "<note><pitch><step>C</step>",
    '<harmony><root><root-step>C</root-step><root-alter>1</root-alter></root><kind>dominant-13th</kind><staff>1</staff></harmony><note><pitch><step>C</step>',
  )
  .replace(
    "<note><rest/>",
    '<harmony><root><root-step>D</root-step><root-alter>-1</root-alter></root><kind>dominant-13th</kind><staff>1</staff></harmony><note><rest/>',
  );
for (const sourceFormat of ["musicxml", "mxl"] as const) {
  let dominantThirteenthEventIds = 0;
  const dominantThirteenthDraft = createLocalScoreProjectMusicXmlImportDraft({
    xml: strictDominantThirteenthHarmonyXml,
    fileName: `严格属十三和弦.${sourceFormat}`,
    sourceFormat,
    projectId: `import-project-dominant-thirteenth-${sourceFormat}`,
    now: "2026-08-01T10:00:00.000Z",
    createEventId: () =>
      `dominant-thirteenth-${sourceFormat}-${++dominantThirteenthEventIds}`,
  });
  assert.equal(dominantThirteenthDraft.status, "ready");
  assert.deepEqual(dominantThirteenthDraft.issues, []);
  assert.deepEqual(
    dominantThirteenthDraft.project?.document.parts[0].staves[0].voices[0]
      .measures.flatMap((measure) => measure.events)
      .map((event) => event.chordSymbol),
    ["C#13", "Db13", null, null, null],
    `${sourceFormat} must preserve dominant-13th symbols`,
  );
  assert.equal(dominantThirteenthEventIds, 5);
}

const strictMajorThirteenthHarmonyXml = supportedXml
  .replace(
    "<note><pitch><step>C</step>",
    '<harmony><root><root-step>C</root-step><root-alter>1</root-alter></root><kind>major-13th</kind><staff>1</staff></harmony><note><pitch><step>C</step>',
  )
  .replace(
    "<note><rest/>",
    '<harmony><root><root-step>D</root-step><root-alter>-1</root-alter></root><kind>major-13th</kind><staff>1</staff></harmony><note><rest/>',
  );
for (const sourceFormat of ["musicxml", "mxl"] as const) {
  let majorThirteenthEventIds = 0;
  const majorThirteenthDraft = createLocalScoreProjectMusicXmlImportDraft({
    xml: strictMajorThirteenthHarmonyXml,
    fileName: `严格大十三和弦.${sourceFormat}`,
    sourceFormat,
    projectId: `import-project-major-thirteenth-${sourceFormat}`,
    now: "2026-08-01T11:00:00.000Z",
    createEventId: () =>
      `major-thirteenth-${sourceFormat}-${++majorThirteenthEventIds}`,
  });
  assert.equal(majorThirteenthDraft.status, "ready");
  assert.deepEqual(majorThirteenthDraft.issues, []);
  assert.deepEqual(
    majorThirteenthDraft.project?.document.parts[0].staves[0].voices[0]
      .measures.flatMap((measure) => measure.events)
      .map((event) => event.chordSymbol),
    ["C#maj13", "Dbmaj13", null, null, null],
    `${sourceFormat} must preserve major-13th symbols`,
  );
  assert.equal(majorThirteenthEventIds, 5);
}

const strictMinorThirteenthHarmonyXml = supportedXml
  .replace(
    "<note><pitch><step>C</step>",
    '<harmony><root><root-step>C</root-step><root-alter>1</root-alter></root><kind>minor-13th</kind><staff>1</staff></harmony><note><pitch><step>C</step>',
  )
  .replace(
    "<note><rest/>",
    '<harmony><root><root-step>D</root-step><root-alter>-1</root-alter></root><kind>minor-13th</kind><staff>1</staff></harmony><note><rest/>',
  );
for (const sourceFormat of ["musicxml", "mxl"] as const) {
  let minorThirteenthEventIds = 0;
  const minorThirteenthDraft = createLocalScoreProjectMusicXmlImportDraft({
    xml: strictMinorThirteenthHarmonyXml,
    fileName: `严格小十三和弦.${sourceFormat}`,
    sourceFormat,
    projectId: `import-project-minor-thirteenth-${sourceFormat}`,
    now: "2026-08-01T12:00:00.000Z",
    createEventId: () =>
      `minor-thirteenth-${sourceFormat}-${++minorThirteenthEventIds}`,
  });
  assert.equal(minorThirteenthDraft.status, "ready");
  assert.deepEqual(minorThirteenthDraft.issues, []);
  assert.deepEqual(
    minorThirteenthDraft.project?.document.parts[0].staves[0].voices[0]
      .measures.flatMap((measure) => measure.events)
      .map((event) => event.chordSymbol),
    ["C#m13", "Dbm13", null, null, null],
    `${sourceFormat} must preserve minor-13th symbols`,
  );
  assert.equal(minorThirteenthEventIds, 5);
}
...
const invalidHarmonies = [
  ...
  '<harmony><root><root-step>C</root-step></root><kind text="5">power</kind><staff>1</staff></harmony>',
  ...
  '<harmony><root><root-step>C</root-step></root><kind text="maj11">major-11th</kind><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>Major-11th</kind><staff>1</staff></harmony>',
  '<harmony placement="above"><root><root-step>C</root-step></root><kind>major-11th</kind><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step><root-alter>2</root-alter></root><kind>major-11th</kind><staff>1</staff></harmony>',
  '<harmony><kind>major-11th</kind><root><root-step>C</root-step></root><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>major-11th</kind><bass><bass-step>G</bass-step></bass><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>major-11th</kind><inversion>1</inversion><staff>1</staff></harmony>',
  '<harmony xmlns="urn:unsupported"><root><root-step>C</root-step></root><kind>major-11th</kind><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>major-11th</kind><staff>1</staff><degree><degree-value>5</degree-value><degree-alter>1</degree-alter><degree-type>alter</degree-type></degree></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind text="m11">minor-11th</kind><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>Minor-11th</kind><staff>1</staff></harmony>',
  '<harmony placement="above"><root><root-step>C</root-step></root><kind>minor-11th</kind><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step><root-alter>2</root-alter></root><kind>minor-11th</kind><staff>1</staff></harmony>',
  '<harmony><kind>minor-11th</kind><root><root-step>C</root-step></root><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>minor-11th</kind><bass><bass-step>G</bass-step></bass><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>minor-11th</kind><inversion>1</inversion><staff>1</staff></harmony>',
  '<harmony xmlns="urn:unsupported"><root><root-step>C</root-step></root><kind>minor-11th</kind><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>minor-11th</kind><staff>1</staff><degree><degree-value>5</degree-value><degree-alter>1</degree-alter><degree-type>alter</degree-type></degree></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind text="13">dominant-13th</kind><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>Dominant-13th</kind><staff>1</staff></harmony>',
  '<harmony placement="above"><root><root-step>C</root-step></root><kind>dominant-13th</kind><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step><root-alter>2</root-alter></root><kind>dominant-13th</kind><staff>1</staff></harmony>',
  '<harmony><kind>dominant-13th</kind><root><root-step>C</root-step></root><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>dominant-13th</kind><bass><bass-step>G</bass-step></bass><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>dominant-13th</kind><inversion>1</inversion><staff>1</staff></harmony>',
  '<harmony xmlns="urn:unsupported"><root><root-step>C</root-step></root><kind>dominant-13th</kind><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>dominant-13th</kind><staff>1</staff><degree><degree-value>5</degree-value><degree-alter>1</degree-alter><degree-type>alter</degree-type></degree></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind text="maj13">major-13th</kind><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>Major-13th</kind><staff>1</staff></harmony>',
  '<harmony placement="above"><root><root-step>C</root-step></root><kind>major-13th</kind><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step><root-alter>2</root-alter></root><kind>major-13th</kind><staff>1</staff></harmony>',
  '<harmony><kind>major-13th</kind><root><root-step>C</root-step></root><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>major-13th</kind><bass><bass-step>G</bass-step></bass><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>major-13th</kind><inversion>1</inversion><staff>1</staff></harmony>',
  '<harmony xmlns="urn:unsupported"><root><root-step>C</root-step></root><kind>major-13th</kind><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>major-13th</kind><staff>1</staff><degree><degree-value>5</degree-value><degree-alter>1</degree-alter><degree-type>alter</degree-type></degree></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind text="m13">minor-13th</kind><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>Minor-13th</kind><staff>1</staff></harmony>',
  '<harmony placement="above"><root><root-step>C</root-step></root><kind>minor-13th</kind><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step><root-alter>2</root-alter></root><kind>minor-13th</kind><staff>1</staff></harmony>',
  '<harmony><kind>minor-13th</kind><root><root-step>C</root-step></root><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>minor-13th</kind><bass><bass-step>G</bass-step></bass><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>minor-13th</kind><inversion>1</inversion><staff>1</staff></harmony>',
  '<harmony xmlns="urn:unsupported"><root><root-step>C</root-step></root><kind>minor-13th</kind><staff>1</staff></harmony>',
  '<harmony><root><root-step>C</root-step></root><kind>minor-13th</kind><staff>1</staff><degree><degree-value>5</degree-value><degree-alter>1</degree-alter><degree-type>alter</degree-type></degree></harmony>',
