...
const requiredSources = [
  ...
  "lib/practice/localMelodyNumberedNotationDraft.ts",
...
const localCoursePathSource = readFileSync(
  join(root, "lib/learning/localCoursePath.ts"),
  "utf8",
);
const courseProgressStorageSource = readFileSync(
  join(root, "mobile/src/runtime/mobileCourseProgressStorage.ts"),
  "utf8",
);
const coursePathPanelSource = readFileSync(
  join(root, "mobile/src/LocalCoursePathPanel.tsx"),
  "utf8",
);
...
const melodyDictationSource = readFileSync(
  join(root, "lib/practice/localEarTrainingMelodyDictation.ts"),
  "utf8",
);
const melodyPianoActivitySource = readFileSync(
  join(root, "lib/activity/melodyPianoActivityAdapter.ts"),
  "utf8",
);
const melodyPianoNoteEventSource = readFileSync(
...
const melodyNumberedNotationDraftSource = readFileSync(
  join(root, "lib/practice/localMelodyNumberedNotationDraft.ts"),
  "utf8",
);
const melodyNumberedNotationActivitySource = readFileSync(
  join(root, "lib/activity/melodyNumberedNotationActivityAdapter.ts"),
  "utf8",
);
const melodyImitationSource = readFileSync(
  join(root, "lib/practice/localMelodyImitation.ts"),
  "utf8",
);
const melodyImitationActivitySource = readFileSync(
...
const melodyNumberedNotationInputSource = readFileSync(
  join(root, "components/practice/MelodyNumberedNotationInput.tsx"),
  "utf8",
);
...
if (
  !localCoursePathSource.includes('LOCAL_COURSE_CATALOG_SCHEMA_VERSION = "local-course-catalog-v1"')
  || !localCoursePathSource.includes('LOCAL_COURSE_PROGRESS_SCHEMA_VERSION = "local-course-progress-v1"')
  || !localCoursePathSource.includes('kind: "checked-activity-result-v1"')
  || !localCoursePathSource.includes('assessmentMode: "non-scoring"')
  || !localCoursePathSource.includes("checked-local-course-activity-result-v1")
  || !localCoursePathSource.includes("activityMappingId")
  || !localCoursePathSource.includes("completeLocalCourseLesson")
  || !localCoursePathSource.includes("getLocalCourseLessonAvailability")
  || !courseProgressStorageSource.includes("solfeggio.mobile.course-progress.v1")
  || !courseProgressStorageSource.includes("loadMobileCourseProgress")
  || !courseProgressStorageSource.includes("saveMobileCourseProgress")
  || !courseProgressStorageSource.includes("resetMobileCourseProgress")
  || !coursePathPanelSource.includes("不生成分数、等级或通过／失败")
  || !mobileApp.includes('"course"')
  || !mobileApp.includes("activeCourseRunRef")
  || !mobileApp.includes("originatingCourseRun")
) {
  throw new Error("Android P118a 中文课程目录、顺序解锁、非评分完成或本机版本化进度边界不完整");
}
if (
...
if (
  !melodyDictationSource.includes('"f-sharp-4"')
  || !melodyDictationSource.includes("c5")
  || !melodyPianoActivitySource.includes('"piano" as const')
  || !melodyPianoActivitySource.includes('assessmentMode: "non-scoring"')
  ...
  || !melodyDictationPanelSource.includes("P117c · 本地旋律听写")
  ...
  || !melodyDictationPanelSource.includes("pianoInputEventsRef")
...
if (
  !melodyNumberedNotationDraftSource.includes('schemaVersion: "melody-numbered-notation-draft-v1"')
  || !melodyNumberedNotationDraftSource.includes('MELODY_NUMBERED_NOTATION_CONTEXT = "numbered-notation-fixed-c-v1"')
  || !melodyNumberedNotationDraftSource.includes("checkedFingerprint")
  || !melodyNumberedNotationActivitySource.includes('"numbered-notation" as const')
  || !melodyNumberedNotationActivitySource.includes('assessmentMode: "non-scoring"')
  || !melodyNumberedNotationActivitySource.includes("playbackQualificationId")
  || !scoreDocumentSource.includes('documentKind: "melody-dictation-numbered-answer"')
  || !scoreDocumentSource.includes('kind: "confirmed-melody-numbered-notation-draft"')
  || !melodyNumberedNotationInputSource.includes("固定 C 为 1")
  || !melodyNumberedNotationInputSource.includes("melody-numbered-octave-dot")
  || !melodyNumberedNotationInputSource.includes("melody-numbered-sharp")
  || !melodyNumberedNotationInputSource.includes("检查简谱草稿")
  || !melodyNumberedNotationInputSource.includes("确认当前简谱修订")
  || !melodyDictationPanelSource.includes("P117c · 本地旋律听写")
  || !melodyDictationPanelSource.includes("检查本轮简谱答案")
  || !melodyDictationPanelSource.includes("numberedNotationAnswerAvailable = pianoAnswerAvailable")
  || !mobileApp.includes("五种受控方式听写")
) {
  throw new Error("Android P117c 旋律听写简谱草稿、真实简谱表示、确认文档、非评分 Activity 或来源完整性边界不完整");
}
if (
  !melodyImitationSource.includes('LOCAL_MELODY_IMITATION_TIMELINE_VERSION = "local-melody-imitation-timeline-v1"')
  || !melodyImitationSource.includes("LOCAL_MELODY_IMITATION_COUNT_IN_BEATS = 4")
  || !melodyImitationSource.includes("recordingZeroMs")
  || !melodyImitationSource.includes("getLocalMelodyImitationP113Targets")
  || !melodyImitationActivitySource.includes('family: "melody-imitation"')
  || !melodyImitationActivitySource.includes('allowedInputModes: ["microphone"]')
  || !melodyImitationActivitySource.includes('assessmentMode: "non-scoring"')
  || !melodyImitationActivitySource.includes("playbackQualificationId")
  || !melodyImitationActivitySource.includes("recordingId")
  || !melodyImitationActivitySource.includes("analysisRunId")
  || !melodyImitationPanelSource.includes("P117d · 会话内非评分练习")
  || !melodyImitationPanelSource.includes("完整播放隐藏旋律")
  || !melodyImitationPanelSource.includes("开始四拍预备与录音")
  || !melodyImitationPanelSource.includes("<OfflinePitchAnalysisPanel")
  || !melodyImitationPanelSource.includes("onAnalysisReady={handleAnalysisReady}")
  || !melodyImitationPanelSource.includes("onAnalysisFailed=")
  || !melodyImitationPanelSource.includes("monitor.hasCompletedRecordingPlayback")
  || !melodyImitationPanelSource.includes("expectedRecordingZero")
  || !melodyImitationPanelSource.includes("showTarget={false}")
...
console.log("Android 本地模式校验通过：固定包名、本地资源、既有十类练习、P116a 节奏视读与会话校准、P116b 隐藏目标节奏回模、P116c 单一事件节奏找错、P116d 谱面草稿节奏听写、P117a 旋律听写屏幕钢琴答案、P117b 五线谱答案文档、P117c 固定 C 简谱答案文档、P117d 隐藏三音麦克风回唱、P117e 可见三音谱面视唱、P118a 版本化中文课程路径与本机顺序进度、音程比较/非评分模唱反馈、实时音高反馈、本机复练、非评分学习画像、本地参考钢琴、无远程运行时配置与生命周期保护。");
