      ...
      <section className="mx-auto max-w-4xl rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 sm:p-8">
        ...
        <section className="mt-6 rounded-3xl border border-violet-200 bg-violet-50 p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <button
              type="button"
              onClick={handleGenerateLocalTargetPitchCurveDraft}
              disabled={!localMelodyGuideDecodedAudio?.analysisReady}
          ...
          <div className="mt-5 grid gap-3 text-sm md:grid-cols-4">
            <div className="rounded-2xl bg-white p-4 ring-1 ring-violet-200">
              ...
              <p className="mt-2 text-violet-800">{localTargetPitchCurveDraft?.status ?? (localMelodyGuideDecodedAudio?.analysisReady ? "ready" : "waiting for decoded guide")}</p>
            </div>
            <div className="rounded-2xl bg-white p-4 ring-1 ring-violet-200">
              <p className="font-semibold text-violet-950">Frames</p>
              <p className="mt-2 text-violet-800">{localTargetPitchCurveDraft ? `${localTargetPitchCurveDraft.frameCount} total` : "—"}</p>
            </div>
            <div className="rounded-2xl bg-white p-4 ring-1 ring-violet-200">
              <p className="font-semibold text-violet-950">Voiced / unvoiced</p>
              <p className="mt-2 text-violet-800">{localTargetPitchCurveDraft ? `${localTargetPitchCurveDraft.voicedFrameCount} / ${localTargetPitchCurveDraft.unvoicedFrameCount}` : "—"}</p>
            </div>
            <div className="rounded-2xl bg-white p-4 ring-1 ring-violet-200">
              <p className="font-semibold text-violet-950">Duration</p>
              <p className="mt-2 text-violet-800">{localTargetPitchCurveDraft ? `${(localTargetPitchCurveDraft.durationMs / 1000).toFixed(2)}s` : "—"}</p>
            </div>
          </div>
            ...
            <div className="mt-5 grid gap-3 text-sm lg:grid-cols-2">
              <div className="rounded-2xl border border-violet-200 bg-white p-4">
                ...
                <p className="mt-2 text-violet-800">
                  Min {localTargetPitchCurveDraft.frequencyMinHz === null ? "—" : `${localTargetPitchCurveDraft.frequencyMinHz.toFixed(1)} Hz`} · Median {localTargetPitchCurveDraft.frequencyMedianHz === null ? "—" : `${localTargetPitchCurveDraft.frequencyMedianHz.toFixed(1)} Hz`} · Max {localTargetPitchCurveDraft.frequencyMaxHz === null ? "—" : `${localTargetPitchCurveDraft.frequencyMaxHz.toFixed(1)} Hz`}
                </p>
                <p className="mt-3 font-semibold text-violet-950">First draft frames</p>
                <ul className="mt-2 space-y-1 text-violet-800">
                  {localTargetPitchCurveDraft.frames.slice(0, 5).map((frame) => (
              ...
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
                ...
                <ul className="mt-2 list-disc space-y-1 pl-5 text-amber-800">
                  {localTargetPitchCurveDraft.warnings.map((warning) => (
