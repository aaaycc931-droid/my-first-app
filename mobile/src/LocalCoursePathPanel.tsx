...
export function LocalCoursePathPanel() {
  ...
  const complete = (event: ChoiceActivityCheckedEvent) => {
    ...
    try { const next = recordLocalCourseLessonCheck(progress, activeLesson, event); const saved = saveMobileCourseProgress(browserStorage(), next); if (saved.notice) { setNotice(saved.notice); return; } setProgress(next); setNotice("已记录：你完成了本课节的核对。结果不是分数、等级或通过判断。"); }
  ...
  return <section className="grid gap-4" aria-label="中文课程路径">
    ...
    <div><button type="button" onClick={() => setConfirmReset(true)} className="min-h-11 rounded-xl border border-rose-300 bg-white px-4 py-2 text-sm font-bold text-rose-800">重置课程进度</button>{confirmReset ? <div className="mt-3 rounded-2xl border border-rose-200 bg-rose-50 p-4" role="alert"><p className="font-bold">确认清除本机课程进度？</p><div className="mt-3 flex gap-2"><button type="button" onClick={() => { const result = clearMobileCourseProgress(browserStorage()); setConfirmReset(false); if (result.notice) { setNotice(result.notice); return; } setProgress(createEmptyLocalCourseProgress()); setNotice("课程进度已重置。"); }} className="min-h-11 rounded-xl bg-rose-700 px-4 py-2 text-sm font-bold text-white">确认重置</button><button type="button" onClick={() => setConfirmReset(false)} className="min-h-11 rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-bold">取消</button></div></div> : null}</div>
