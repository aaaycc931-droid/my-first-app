...
import { adaptModulationQuestionToActivity } from "../../lib/activity/legacyLocalActivityAdapter";
import {
...
export function LocalEarTrainingModulationPanel({
  ...
  const activityDefinition = useMemo(
    () => adaptModulationQuestionToActivity(question),
    [question],
  );
  const activity = useChoiceActivitySession(
    activityDefinition,
    `modulation:${question.id}`,
  );
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <p className="text-sm font-semibold tracking-wide text-blue-700">P115 本地和声练耳</p>
      <h2 className="mt-1 text-2xl font-bold text-slate-950">调制方向听辨</h2>
      <p className="mt-2 text-sm leading-6 text-slate-600">
        听一组依次播放的和弦，判断音乐从当前调性转向哪里。只提供答案核对与调性线索解释，不生成正式分数。
      </p>

      <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.25fr)]">
        <div className="rounded-2xl bg-blue-50 p-4 ring-1 ring-blue-100">
          <label className="block text-sm font-semibold text-slate-800" htmlFor="modulation-training-difficulty">
            练习难度
          </label>
          <select
            ...
            className="mt-2 w-full rounded-xl border border-blue-200 bg-white px-3 py-2 text-slate-900 disabled:bg-slate-100"
          >
            <option value="基础">基础：近关系调方向</option>
            <option value="进阶">进阶：常见转调关系</option>
            <option value="挑战">挑战：多种调制线索</option>
          </select>
          <p className="mt-4 text-sm leading-6 text-blue-950">
            本难度共 {variantCount} 个版本化组合；当前为第 {sequence + 1} 题。完整一轮出现前不重复。
          </p>
          {!isReady ? <p className="mt-2 text-sm text-blue-800">正在准备本轮题目…</p> : null}
          <button
            type="button"
            disabled={!isReady || isPlaying}
            onClick={() => void playQuestion()}
            className="mt-4 w-full rounded-xl bg-blue-700 px-4 py-3 font-bold text-white disabled:bg-blue-300"
          >
            {playbackState === "准备中" ? "正在准备声音…" : isPlaying ? "正在播放调制进行…" : "播放调制进行"}
          </button>
          <button
            type="button"
            disabled={!isPlaying}
            onClick={stopPlayback}
            className="mt-2 w-full rounded-xl border border-blue-300 bg-white px-4 py-3 font-bold text-blue-900 disabled:opacity-50"
          >
            停止播放
          </button>
          {audioError ? (
            <p className="mt-3 rounded-xl bg-rose-50 p-3 text-sm leading-6 text-rose-800">{audioError}</p>
          ) : null}
        </div>

        <div className="rounded-2xl border border-slate-200 p-4">
          <p className="text-sm font-semibold text-slate-500">回答本题</p>
          <p className="mt-1 text-lg font-bold text-slate-950">选择听到的调制方向</p>
          <ActivityChoiceAnswerPanel
            options={options}
            selectedOptionId={answerLock.selection}
            disabled={!isReady || answerLock.isAnswerVisible}
            accent="sky"
            columns={2}
            onChoose={(optionId) => {
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button type="button" disabled={!answer.hasSelection} onClick={revealAnswer} className="rounded-xl bg-slate-900 px-4 py-2.5 font-bold text-white disabled:bg-slate-300">
              查看本题答案
            </button>
            {answerLock.isAnswerVisible && !answer.matchesAnswer ? (
              <button type="button" onClick={() => { resetCurrentQuestion(); void playQuestion(); }} className="rounded-xl border border-amber-300 bg-amber-50 px-4 py-2.5 font-bold text-amber-900">
                重新播放并复练本题
              </button>
            ) : null}
            <button type="button" onClick={resetCurrentQuestion} className="rounded-xl border border-slate-300 bg-white px-4 py-2.5 font-bold text-slate-800">
              重置本题
            </button>
            <button type="button" disabled={!isReady} onClick={nextQuestion} className="rounded-xl border border-blue-300 bg-white px-4 py-2.5 font-bold text-blue-900 disabled:opacity-50">
              {initialReviewTarget ? "返回随机练习" : "下一题"}
            </button>
          </div>
          {!answer.hasSelection ? <p className="mt-3 text-sm leading-6 text-slate-500">请先选择一个调制方向，再查看答案。</p> : null}
      ...
      {showLocalPiano ? (
        <section className="mt-5 rounded-2xl border border-blue-200 bg-blue-50 p-4" aria-label="调制听辨参考钢琴">
          <button type="button" aria-expanded={isLocalPianoOpen} aria-controls="modulation-reference-piano" onClick={() => setIsLocalPianoOpen((current) => !current)} className="flex min-h-11 w-full items-center justify-between rounded-xl bg-white px-4 py-3 text-left font-bold text-blue-950 ring-1 ring-blue-200">
            <span>参考钢琴</span><span className="text-sm">{isLocalPianoOpen ? "收起" : "展开"}</span>
          </button>
          <p className="mt-2 text-sm leading-6 text-blue-900">仅用于本地寻找调性线索；弹奏不保存、不上传，也不参与答案判定。</p>
          {isLocalPianoOpen ? <div id="modulation-reference-piano" className="mt-4"><LocalPianoPanel /></div> : null}
        </section>
      ) : null}
