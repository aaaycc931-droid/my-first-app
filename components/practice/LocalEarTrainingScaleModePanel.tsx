...
import { adaptScaleModeQuestionToActivity } from "../../lib/activity/legacyLocalActivityAdapter";
import {
...
export function LocalEarTrainingScaleModePanel({
  initialReviewTarget,
  onLocalAnswerResult,
  onLeaveReviewTarget,
  ...
  const question = useMemo(() => createLocalScaleModeQuestion({
    difficulty,
    sequence,
    questionIndex,
  ...
  const activityDefinition = useMemo(() => adaptScaleModeQuestionToActivity(question), [question]);
  const activity = useChoiceActivitySession(activityDefinition, `scale:${question.id}`);

  const playQuestion = async () => {
    setAudioError("");
    const playbackError = await play((audioContext, channel) => {
      const startTime = audioContext.currentTime + 0.05;
      question.frequenciesHz.forEach((frequencyHz, index) => {
        const oscillator = audioContext.createOscillator();
        const gain = audioContext.createGain();
        const noteStart = startTime + index * 0.31;
        const noteEnd = noteStart + 0.27;
      ...
      return question.frequenciesHz.length * 310 + 100;
    });
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <p className="text-sm font-semibold tracking-wide text-teal-700">P115 本地调式练耳</p>
      <h2 className="mt-1 text-2xl font-bold text-slate-950">音阶与调式听辨</h2>
      <p className="mt-2 text-sm leading-6 text-slate-600">听从主音上行到高八度的音阶，辨认音阶或调式类型。只核对答案并解释特征音，不生成正式分数。</p>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.3fr)]">
        <div className="rounded-2xl bg-teal-50 p-4 ring-1 ring-teal-100">
          ...
          <select id="scale-mode-difficulty" value={difficulty} disabled={Boolean(initialReviewTarget)} onChange={(event) => {
            ...
            answerLock.reset();
            setAudioError("");
          }} className="mt-2 w-full rounded-xl border border-teal-200 bg-white px-3 py-2 text-slate-900 disabled:bg-slate-100">
            <option value="基础">基础：大小调与大小调五声</option>
            <option value="进阶">进阶：小调变化与常见调式</option>
            <option value="挑战">挑战：完整教会调式与全音音阶</option>
          </select>
          <p className="mt-4 text-sm leading-6 text-teal-950">本难度共 {variantCount} 个版本化组合；当前为第 {sequence + 1} 题。完整一轮出现前不重复。</p>
          {!isReady ? <p className="mt-2 text-sm text-teal-800">正在准备本轮题目…</p> : null}
          <button type="button" disabled={!isReady || isPlaying} onClick={() => void playQuestion()} className="mt-4 w-full rounded-xl bg-teal-700 px-4 py-3 font-bold text-white disabled:bg-teal-300">{playbackState === "准备中" ? "正在准备声音…" : isPlaying ? "正在播放音阶…" : "播放上行音阶"}</button>
          <button type="button" disabled={!isPlaying} onClick={stopPlayback} className="mt-2 w-full rounded-xl border border-teal-300 bg-white px-4 py-3 font-bold text-teal-900 disabled:opacity-50">停止播放</button>
        ...
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <p className="mt-1 text-lg font-bold text-slate-950">选择听到的音阶或调式</p>
          <ActivityChoiceAnswerPanel options={options} selectedOptionId={answerLock.selection} disabled={!isReady || answerLock.isAnswerVisible} accent="emerald" columns={2} onChoose={(optionId) => {
            if (!answerLock.choose(optionId as ScaleModeId)) return;
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <button type="button" disabled={!isReady} onClick={nextQuestion} className="rounded-xl border border-teal-300 bg-white px-4 py-2.5 font-bold text-teal-900 disabled:opacity-50">{initialReviewTarget ? "返回随机练习" : "下一题"}</button>
          </div>
          {!answer.hasSelection ? <p className="mt-3 text-sm leading-6 text-slate-500">请先选择一个音阶或调式，再查看答案。</p> : null}
      ...
      {showLocalPiano ? <section className="mt-5 rounded-2xl border border-teal-200 bg-teal-50 p-4" aria-label="音阶调式参考钢琴">
        <button type="button" aria-expanded={isLocalPianoOpen} aria-controls="scale-mode-reference-piano" onClick={() => setIsLocalPianoOpen((current) => !current)} className="flex min-h-11 w-full items-center justify-between rounded-xl bg-white px-4 py-3 text-left font-bold text-teal-950 ring-1 ring-teal-200"><span>参考钢琴</span><span className="text-sm">{isLocalPianoOpen ? "收起" : "展开"}</span></button>
        <p className="mt-2 text-sm leading-6 text-teal-900">仅用于本地找音；弹奏不保存、不上传，也不参与答案判定。</p>
        {isLocalPianoOpen ? <div id="scale-mode-reference-piano" className="mt-4"><LocalPianoPanel /></div> : null}
      </section> : null}
