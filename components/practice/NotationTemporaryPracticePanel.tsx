...
export function NotationTemporaryPracticePanel({ target, onGoToSheetMusic, onClear, onPracticeCurrentNote, onPracticeRhythmTarget, progress, onToggleEventCompletion, onRestartPracticeRound }: Props) {
  ...
  return (
    <section className="mt-6 rounded-3xl border border-fuchsia-200 bg-fuchsia-50 p-5 shadow-sm sm:p-6">
      ...
      {isSightSinging && !isRest ? <div className="mt-5 rounded-2xl border border-violet-200 bg-violet-50 p-4 text-left text-sm text-violet-950"><p className="font-semibold">当前视唱音符的本地参考与跟练</p><p className="mt-1 leading-6">可主动播放一次短参考音，或进入现有的浏览器本地录音与音高估计查看参考提示。不会自动播放、自动打开麦克风，也不是正式评分。</p><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => void playReferenceTone()} disabled={isReferenceTonePlaying || referenceToneHz === null} className="rounded-full border border-violet-300 px-4 py-2 font-semibold text-violet-800 disabled:cursor-not-allowed disabled:border-slate-200 disabled:text-slate-400">{isReferenceTonePlaying ? "正在播放参考音" : "播放当前参考音"}</button><button type="button" onClick={() => onPracticeCurrentNote(event, eventIndex)} className="rounded-full bg-violet-700 px-4 py-2 font-semibold text-white">使用当前音符进行本地跟练</button></div>{referenceToneError ? <p className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-amber-900">{referenceToneError}</p> : null}</div> : !isSightSinging ? <div className="mt-5 rounded-2xl border border-indigo-200 bg-indigo-50 p-4 text-left text-sm text-indigo-950"><p className="font-semibold">此临时节奏目标的本地拍击练习</p><p className="mt-1 leading-6">主动进入后，可复用现有浏览器节拍器与 tap / 空格键输入，按非休止事件的起点获得非评分节奏提示。不会打开麦克风，不会生成正式成绩。</p><button type="button" onClick={onPracticeRhythmTarget} className="mt-3 rounded-full bg-indigo-700 px-4 py-2 font-semibold text-white">使用此临时节奏目标进行拍击练习</button></div> : <p className="mt-5 rounded-2xl border border-slate-200 bg-white p-4 text-sm leading-6 text-slate-700">休止事件不需要音高跟练；请保持休止并继续下一个事件。</p>}
