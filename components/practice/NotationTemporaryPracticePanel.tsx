...
export function NotationTemporaryPracticePanel({ target, onGoToSheetMusic, onClear, onPracticeCurrentNote, onPracticeRhythmTarget, progress, onToggleEventCompletion, onRestartPracticeRound }: Props) {
  ...
  return (
    <section className="mt-6 rounded-3xl border border-fuchsia-200 bg-fuchsia-50 p-5 shadow-sm sm:p-6">
      ...
      {isSightSinging ? <div className="mt-5 rounded-2xl border border-violet-200 bg-violet-50 p-4 text-left text-sm text-violet-950"><p className="font-semibold">本地参考与跟练</p><p className="mt-1 leading-6">可主动播放整段旋律或当前小节；播放会保留休止时长。当前音符不是休止时，还可播放一次短参考音或进入现有的浏览器本地录音与音高估计查看参考提示。播放中可随时停止。不会自动播放、自动打开麦克风，也不是正式评分。</p><div className="mt-3 flex flex-wrap gap-3"><label className="flex items-center gap-2 font-semibold text-violet-900">播放范围<select value={referenceMelodyScope} disabled={isReferenceTonePlaying} onChange={(change) => { stopReferenceTone(); setReferenceMelodyScope(change.target.value as "full" | "current-measure"); }} className="rounded-lg border border-violet-300 bg-white px-2 py-1 font-normal disabled:border-slate-200"><option value="full">完整旋律</option><option value="current-measure">当前第 {event.measure} 小节</option></select></label><label className="flex items-center gap-2 font-semibold text-violet-900">速度<select value={referenceMelodyRate} disabled={isReferenceTonePlaying} onChange={(change) => { stopReferenceTone(); setReferenceMelodyRate(Number(change.target.value) as NotationReferenceMelodyPlaybackRate); }} className="rounded-lg border border-violet-300 bg-white px-2 py-1 font-normal disabled:border-slate-200"><option value={0.75}>慢速（75%）</option><option value={1}>标准（100%）</option></select></label></div><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => void playReferenceMelody()} disabled={isReferenceTonePlaying} className="rounded-full border border-violet-300 px-4 py-2 font-semibold text-violet-800 disabled:cursor-not-allowed disabled:border-slate-200 disabled:text-slate-400">{referencePlaybackKind === "melody" ? "正在播放参考旋律" : referenceMelodyScope === "current-measure" ? "播放当前小节参考" : "播放完整参考旋律"}</button>{!isRest ? <><button type="button" onClick={() => void playReferenceTone()} disabled={isReferenceTonePlaying || referenceToneHz === null} className="rounded-full border border-violet-300 px-4 py-2 font-semibold text-violet-800 disabled:cursor-not-allowed disabled:border-slate-200 disabled:text-slate-400">{referencePlaybackKind === "tone" ? "正在播放参考音" : "播放当前参考音"}</button><button type="button" onClick={() => onPracticeCurrentNote(event, eventIndex)} className="rounded-full bg-violet-700 px-4 py-2 font-semibold text-white">使用当前音符进行本地跟练</button></> : null}{isReferenceTonePlaying ? <button type="button" onClick={stopReferenceTone} className="rounded-full border border-rose-300 bg-white px-4 py-2 font-semibold text-rose-700">停止参考播放</button> : null}</div>{isRest ? <p className="mt-3 text-slate-700">当前是休止事件，不提供单音跟练；完整或当前小节参考都会保留这段安静时长。</p> : null}{referenceToneError ? <p className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-amber-900">{referenceToneError}</p> : null}</div> : <div className="mt-5 rounded-2xl border border-indigo-200 bg-indigo-50 p-4 text-left text-sm text-indigo-950"><p className="font-semibold">此临时节奏目标的本地拍击练习</p><p className="mt-1 leading-6">主动进入后，可复用现有浏览器节拍器与 tap / 空格键输入，按非休止事件的起点获得非评分节奏提示。不会打开麦克风，不会生成正式成绩。</p><button type="button" onClick={onPracticeRhythmTarget} className="mt-3 rounded-full bg-indigo-700 px-4 py-2 font-semibold text-white">使用此临时节奏目标进行拍击练习</button></div>}
