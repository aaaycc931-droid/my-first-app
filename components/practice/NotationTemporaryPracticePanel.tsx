...
export function NotationTemporaryPracticePanel({ target, onGoToSheetMusic, onClear, onPracticeCurrentNote, onPracticeRhythmTarget, progress, onToggleEventCompletion, onRestartPracticeRound }: Props) {
  ...
  const playReferenceMelody = async () => {
    ...
    try {
      ...
      const plan = getNotationReferenceMelodyPlaybackPlan(target.events, referenceMelodyRate);
      ...
      referenceMelodyTimeoutRef.current = window.setTimeout(() => stopReferenceTone(), (getNotationReferenceMelodyPlaybackDurationSeconds(target.events, referenceMelodyRate) + 0.2) * 1000);
    } catch {
