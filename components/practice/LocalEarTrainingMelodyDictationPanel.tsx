...
type MelodyAnswerMode = "choice" | "solfege" | "piano";
...
export function LocalEarTrainingMelodyDictationPanel({
  ...
  const activityDefinition = useMemo(
    () => {
      ...
      return pianoAnswerAvailable ? enableMelodyPianoInput(definition) : definition;
    },
    [pianoAnswerAvailable, question],
  );
  ...
  return <section className="mt-5 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
    <p className="text-sm font-semibold tracking-wide text-violet-600">{pianoAnswerAvailable ? "P117a · 本地旋律听写" : "本地练习"}</p>
    <h2 className="mt-1 text-2xl font-bold text-slate-950">内置旋律听写练习</h2>
    <p className="mt-2 text-sm leading-6 text-slate-600">先完整听完三音短旋律，再按听到的顺序使用音名、固定唱名{pianoAnswerAvailable ? "或屏幕钢琴" : ""}作答。本模块不上传音频，也不生成正式成绩。{onLocalAnswerResult ? "当前填写和声音不保存；答错时仅保存复现本题所需的最小信息。" : "当前入口不会保存练习记录。"}</p>
    <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
      ...
      <div className="rounded-2xl border border-slate-200 p-4">
        ...
        <fieldset className="mt-4">
          ...
          <div className={`mt-2 grid gap-2 ${pianoAnswerAvailable ? "grid-cols-3" : "grid-cols-2"}`} data-testid="melody-answer-mode">
            ...
            {pianoAnswerAvailable ? <button type="button" role="radio" aria-checked={answerMode === "piano"} onClick={() => changeAnswerMode("piano")} className={`rounded-xl border px-3 py-2 text-sm font-semibold ${answerMode === "piano" ? "border-violet-600 bg-violet-50 text-violet-900 ring-2 ring-violet-200" : "border-slate-200 text-slate-700"}`}>屏幕钢琴</button> : null}
          </div>
        </fieldset>
        <p className="mt-3 text-sm leading-6 text-slate-600">{answerMode === "choice" ? "使用科学音高名填写，例如 C4、F♯4。" : answerMode === "solfege" ? "使用固定唱名填写；低音区 do 与高音 do 会明确区分，升 fa 表示 F♯。" : "完整听题后，先明确开始接收，再按顺序弹出三个屏幕琴键；允许重复音，琴键会在本机短暂发声。"}</p>
        {answerMode === "piano" ? (
