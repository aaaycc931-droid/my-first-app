...
export function LocalEarTrainingRhythmPanel({
  ...
  const activity = useChoiceActivitySession(activityDefinition, `rhythm:${question.id}`, onActivityChecked);
      ...
      {expandedActivity === "choice" ? <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
        ...
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <button type="button" disabled={!isQuestionReady} onClick={nextQuestion} className="rounded-xl border border-violet-300 bg-white px-4 py-2.5 font-semibold text-violet-800 disabled:cursor-not-allowed disabled:opacity-50">{onActivityChecked ? "重新练习本课节" : initialReviewTarget ? "返回随机练习" : "下一题"}</button>
          </div>
